...
__global__ void warpDivergenceExample(int *d_out, int *d_in) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < N) {  // Ensure we don't access out-of-bounds
        // Thread divergence occurs here because of the below if-else condition
...
int main() {
    ...
    warpDivergenceExample<<<gridSize, blockSize>>>(d_out, d_in);
    ...
    printf("Time taken with warp divergence: %.2f ms\n", elapsedTime);
